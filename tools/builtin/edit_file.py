...
# More enhancments -> patch (mutliple edits in one tool call)
class EditParams(BaseModel):
    path:str= Field(..., description="Path of the file to edit (relative to the working directory or absolute path)")
    old_string:str= Field("", description="The exact text to be replaced. Must match exactly (including whitespace and indentation).For new files it willl be left empty.")
    new_string:str= Field(..., description="The new text to replace the old_string with.Can be empty if we want to delete text")
...
class EditTool(Tool):
    ...
    async def execute(self, invocation:ToolInvocation)->ToolResult:
        ...
        if not path.exists():
            ...
            return ToolResult.success_result(
                f"Created {path} with {line_count} lines",
                diff=FileDiff(
                    path=path,
                    old_content="",
                    new_content=params.new_string,
        ...
        # what about mutiple replace of occurenes but not all? 
        if params.replace_all:
            new_content = old_content.replace(params.old_string, params.new_string)
    ...
    def _no_match_error(self, old_string: str, content: str, path: Path) -> ToolResult:
        ...
        partial_matches = []
        search_terms = old_string.split()[:5] #try with first few words to find similar lines if exact match is not found
