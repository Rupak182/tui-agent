...
class MCPClient:
    ...
    def _create_transport(self)->StdioTransport | SSETransport:
        env= os.environ.copy()
        env.update(self.config.env)  # is it safe?
        if self.config.command:
            return StdioTransport(command=self.config.command,args=self.config.args,env=self.config.env,cwd=str(self.config.cwd or self.cwd) )
        else:
    ...
    async def connect(self)->None:
        ...
        try:
            ...
            await self._client.__aenter__()  # want to persist the connection
    ...
    async def call_tool(self,tool_name:str,arguments:dict[str,Any]):
        if not self._client or self.status != MCPServerStatus.CONNECTED:
            raise RuntimeError(f"MCP client is not connected to server {self.name}")
        ...
        return {"output": "\n".join(output), "error": result.is_error}
