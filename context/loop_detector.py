...
class LoopDetector:
    ...
    def record_action(self,action_type:str,**details)->None:
        ...
        self._history.append("|".join(output))
    ...
    def check_for_loop(self)->str|None:
        ...
        if len(self._history)>=self.max_exact_repeats:
            ...
            if len(set(recent))==1:
                return f"Same action repeated {self.max_exact_repeats} times: {recent[0]}"
            
        if len(self._history)>= self.max_cycle_length * 2:
            history_list= list(self._history)

            for cycle_length in range(2,min(self.max_cycle_length+1, len(history_list)//2 +1)):
                recent= history_list[-cycle_length*2:]
                if recent[:cycle_length] == recent[cycle_length:]:
                    return f"Cycle detected of length {cycle_length}"
